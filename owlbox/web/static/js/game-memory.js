// Memory mini-game (match pairs of pictures) - one of several mini-games
// under the Spiele-Menü (see game.js, which owns menu/switching and calls
// window.OwlBoxGameMemory.start()/stop() at the right moments).
(function () {
  const containerEl = document.getElementById("game-memory");
  ...
  // Memory activation. Extra uploaded images beyond a given difficulty's
  ...
  // Tracked so stop() can cancel a pending flip-back/win reveal if the
  // player leaves mid-animation (back button or the RFID chip toggling game
  // mode off) - otherwise a stale timeout could still touch these DOM nodes
  // after the round was meant to have ended.
  ...
  function handleCardTap(card) {
    ...
    if (first.dataset.pairId === second.dataset.pairId) {
      ...
      if (matchedPairs === totalPairs) {
        boardLocked = true;
        pendingTimeout = setTimeout(showWin, 500);
      }
    } else {
      boardLocked = true;
  ...
  async function start() {
    ...
    try {
      ...
      // principle as the round shuffle itself (see game.js).
      showDifficultyScreen();
    } catch (err) {
      emptyHintEl.hidden = false;
    }
  ...
  window.OwlBoxGameMemory = { start, stop };
})();
