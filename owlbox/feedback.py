"""Short acoustic confirmation sounds on RFID scans (chip erkannt / unbekannt /
Funktions-Chip). Played as a fire-and-forget `aplay` subprocess rather than
through the main mpv instance, so an already-playing story is never
interrupted - best-effort: if aplay is missing, or the ALSA device is busy
